...
# bw = 'bw' # Background white
# bb = 'bb' # Background black
# fw = 'fw' # Foreground white
# fb = 'fb' # Foreground black
# fr = 'fr' # Foreground reset
# br = 'br' # Background reset
...
class Player(object):
    ...
    def alphabeta_search(self, state):
        ...
        def min_value(state, alpha, beta, depth):
            ...
            v = inf
            for child in state.children:
                temp = max_value(child, alpha, beta, depth+1)
                v = min(v, temp)
                if v <= alpha: return v
                beta = min(beta, v)
            return v
    ...
    def _heuristic_x(self, state, depth):
        ...
        KY_moves = list(filter(state.king_filter(state.KY), state.y_attacking_positions))
    ...
    def _heuristic_y(self, state, depth):
        ...
        KY_moves = len(list(filter(state.king_filter(state.KY), state.y_attacking_positions)))
...
class Rook(object):
    ...
    def _get_attacking_positions(self):
        ...
        hor = [Position(f, current_rank) for f in range(1,9) if f != current_file]
        ver = [Position(current_file, r) for r in range(1,9) if r != current_rank]
        return hor + ver
...
class Board(object):
    ...
    def __repr__(self):
        ...
        mat = []
        for i in range(4):
            mat.append([B, W, B, W, B, W, B, W])
            mat.append([W, B, W, B, W, B, W, B])

        for p in self.pieces:
            f, r = p.position
            row = 8 - r
            column = f - 1
            mat[row][column] = {'KX':'K', 'KY':'k', 'RX':'R', 'RY':'r'}[str(p)]

        s  = "  -----------------\n"
        for i in range(8,0,-1):
            s += str(i)+"| "+ " ".join(mat[i-1])+" |\n"
        s += "   a b c d e f g h\n"
        return s
